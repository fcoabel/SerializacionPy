...
class App():
    def __init__(self, root):
        ...
        filemenu.add_command(label="Manual de Usuario", font=("Helveltica", "9", "normal"))
        filemenu.add_separator()
        filemenu.add_command(label="Cerrar", command=self.window.quit, font=("Helveltica", "9", "normal"))
        ...
        Label(inbox_frame, text='Nombre', bg="#FFFFFF", font=("Calibri", "11", "normal")).grid(row=0, column=0)
        inbox_name = Entry(inbox_frame, font=("Calibri", "11", "normal"), width=28)
        inbox_name.grid(row=1, column=0)
        inbox_name.focus()

        Label(inbox_frame, text='Apellido', bg="#FFFFFF", font=("Calibri", "11", "normal")).grid(row=0, column=1)
        inbox_Email = Entry(inbox_frame, font=("Calibri", "11", "normal"), width=30)
        inbox_Email.grid(row=1, column=1)

        Label(inbox_frame, text='Número de Teléfono', bg="#FFFFFF", font=("Calibri", "11", "normal")).grid(row=0, column=2)
        inbox_phone = Entry(inbox_frame, font=("Calibri", "11", "normal"), width=20)
        inbox_phone.grid(row=1, column=2)



        Add_contact_button = Button(button_frame, command=lambda: add(), text='Añadir Contacto', width=20)
        Add_contact_button.configure(bg="#D0D0D0", cursor='hand2', font=("Calibri", "10", "normal"))
        ...
        delete_button.configure(bg="#D0D0D0", cursor='hand2', font=("Calibri", "10", "normal"))
        delete_button.grid(row=0, column=2, padx=2, pady=3, sticky=W + E)

        modify_button = Button(button_frame, command=lambda: modify(), text='Modificar contacto')
        modify_button.configure(bg="#D0D0D0", cursor='hand2', font=("Calibri", "10", "normal"))
        ...
        save_changes_button.configure(bg="#D0D0D0", cursor='hand2', font=("Calibri", "10", "normal"))
        save_changes_button.grid(row=1, column=2, padx=2, pady=3, sticky=W + E)


        Label(button_frame, text='Modificar Selección', bg="#FFFFFF", font=("Calibri", "10", "normal")).grid(
        ...
        def _view_csv():
            contacts = orden_alfabetico.alphabetic_order()
...
class TopLevelModify():
    def __init__(self, root, val_modify):
        ...
        Label(text_frame, text=self.name, bg="#FFFFFF", font=("Comic Sans MS", "11", "bold")).grid(row=1, column=0)
        Label(text_frame, text=self.phone, bg="#FFFFFF", font=("Comic Sans MS", "11", "bold")).grid(row=1, column=1)
        Label(text_frame, text=self.email, bg="#FFFFFF", font=("Comic Sans MS", "11", "bold")).grid(row=1, column=2)


        Label(text_frame, text='Escriba un nuevo nombre', bg="#FFFFFF", font=("Comic Sans MS", "11", "normal")).grid(row=2,
                                                                                                              column=0)
        n_inbox_name = Entry(text_frame, font=("Comic Sans MS", "11", "normal"), width=28)
        n_inbox_name.grid(row=3, column=0)
        n_inbox_name.focus()

        Label(text_frame, text='Escriba un nuevo número de teléfono', bg="#FFFFFF", font=("Comic Sans MS", "11", "normal")).grid(row=2,
                                                                                                               column=1)
        n_inbox_phone = Entry(text_frame, font=("Comic Sans MS", "11", "normal"), width=20)
        n_inbox_phone.grid(row=3, column=1)

        Label(text_frame, text='Escriba un nuevo apellido', bg="#FFFFFF", font=("Comic Sans MS", "11", "normal")).grid(row=2,
                                                                                                               column=2)
            ...
            with open('contacts_list.csv', 'w') as f:
                ...
                for i, row in enumerate(reader):
                    if name != row[0]:
                        ...
                        search_button.configure(bg="#D0D0D0", cursor='hand2', font=("Calibri", "10", "normal"))
